...
void ResponseSerializer::start(const ResponseHead& head) {
    head_bytes_.clear();
    head_bytes_ += (head.version == 10) ? "HTTP/1.0 " : "HTTP/1.1 ";
    head_bytes_ += std::to_string(head.status);
    head_bytes_ += ' ';
    head_bytes_ += head.reason;
    head_bytes_ += "\r\n";
...
void ResponseSerializer::provide_body(net::ConstBuffer chunk, bool is_last) {
    if (chunked_) {
...
std::size_t ResponseSerializer::pull(net::MutableBuffer out) {
    std::size_t total = 0;
...
bool ResponseSerializer::done() const { return stage_ == Stage::kDone; }
